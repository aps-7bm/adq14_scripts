...
def scan_loop(args):
    ...
    while True:
        ...
        if epics.caget(args['handshake']) == 1:
            time.sleep(0.5)
            current_point = int(epics.caget(args['scan'] + '.CPT'))
            update_arrays(args, current_point)
            epics.caput(args['handshake'], 0)
            #Handle the end of the scan
            if (current_point + 1 == epics.caget(args['scan'] + '.NPTS')
                or epics.caget(args['scan'] + '.EXSC') == 0):
